// C Program to design DFA that accepts set of all strings ending with 01
#include <stdio.h>
#include<conio.h>
...
int main()
{
	...
	while(choice != 'n' && choice != 'N')
	{
		...
		if(isAccepted == 0) printf("String %s is Rejected", string);
		if(state == -1) printf("String %s is Invalid", string);
		printf("\nContinue? (Y/N): ");
		scanf(" %c", &choice);
	}
	return 0;
